...
>
> 关联章节：[[03_中断与定时器|定时器中断]] · [[07_TIM触发ADC采集|TIM 触发 ADC]] · [[08_TIM通道捕获|输入捕获]] · [[09_电机与工程模板|电机控制]]
...
- [ ] 能按 TIMx_CLK、PSC、ARR 计算 PWM 频率。
- [ ] 能解释 ARR 决定周期、CCR 决定高电平时间。
- [ ] 能区分 PWM 频率、占空比和具体高电平脉宽。
- [ ] 能说明舵机控制为什么关注脉宽而不只是占空比。
- [ ] 能写出启动 PWM 和运行时修改 CCR 的基本流程。
- [ ] 能解释呼吸灯为什么本质上是周期性改变 CCR。
- [ ] 能说明通用定时器和高级定时器在入门实验中的主要区别。
- [ ] 能按引脚复用、TIM 通道、`HAL_TIM_PWM_Start`、CCR 范围排查 PWM 无输出。
