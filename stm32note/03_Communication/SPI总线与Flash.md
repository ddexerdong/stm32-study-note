...
SPI 是由主机时钟驱动的同步移位过程，读取数据时也必须继续发送时钟，常见做法是发 dummy byte。CS 拉低后命令、地址和数据的含义由从设备协议决定；Flash 可靠写入还依赖写使能、擦除、页边界和忙等待。
...
- [[通用调试方法]] 说明片选、模式和事务波形的观察方法。

## 复习检查清单

- [ ] 能说明 SCK、MOSI、MISO 和 CS 在一笔 SPI 事务中的作用。
- [ ] 能按器件手册选择 CPOL/CPHA，而不是把某一种模式当成通用配置。
- [ ] 能解释 SPI 全双工移位以及读取时为何仍需发送时钟或 dummy byte。
- [ ] 能保证手动 CS 覆盖完整的命令、地址和数据事务。
- [ ] 能按读 ID、写使能、页写、BUSY 等待和读回校验验证 Flash。
- [ ] 能说明擦除粒度、页边界、地址宽度和容量必须按具体型号确认。
- [ ] 能用逻辑分析仪检查 CS、模式、命令、地址和返回数据是否对齐。
