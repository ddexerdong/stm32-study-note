...
>
> 关联章节：[[05_ADC模数转换|ADC]] · [[06_定时器PWM|TIM]]
...
- [ ] 能说明 TIM 触发 ADC 的核心是硬件 TRGO，不是软件轮询触发。
- [ ] 能按 TIMx_CLK、PSC、ARR 计算采样节拍。
- [ ] 能解释 `Trigger Output` 选择 `Update Event` 的作用。
- [ ] 能说明 ADC 外部触发源必须和 TIM TRGO 对上。
- [ ] 能区分 ADC 启动、TIM 启动和 Poll 等待结果的关系。
- [ ] 能说明外部触发模式下为什么通常关闭 Continuous。
- [ ] 能解释内部温度传感器测的是芯片温度而不是环境温度。
- [ ] 能按 ADC 先启动、TIM 后启动、TRGO、触发源、DMA 扩展排查问题。
