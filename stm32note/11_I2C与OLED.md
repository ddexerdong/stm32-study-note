...
>
> 关联章节：[[10_DWT与单总线传感器|DHT11]] · [[12_I2C传感器|I2C 传感器]] · [[STM32术语表|术语表]]
...
PDF 可以支撑 I2C 和显示概念，但不能直接补完整 OLED 小模块实验。OLED 控制器、地址、初始化命令和显存组织必须看视频或模块资料。

### 学习目标

理解 OLED 初始化不是“点亮屏幕”这么简单，而是按控制器规则写命令，让屏幕进入可显示状态；清屏则是向显示 RAM 写 0。
...
{
    /* 视频待核对：I2C 地址和控制字节。 */
    HAL_I2C_Master_Transmit(&hi2c1, OLED_I2C_ADDR, &cmd, 1, 100);
}

void OLED_WriteData(uint8_t data)
{
    /* 视频待核对：数据控制字节和连续写入方式。 */
    HAL_I2C_Master_Transmit(&hi2c1, OLED_I2C_ADDR, &data, 1, 100);
}
...
- 先用 I2C 地址扫描确认 OLED 有 ACK。
- 再检查写命令和写数据的控制字节。
- 清屏不成功时，先看页地址和列地址是否设置正确。

### 常见坑

- OLED 地址写法和 HAL 7 位地址左移规则混淆。
- 初始化命令表来自不同控制器，直接套用不匹配。
- 清屏只清了一页或列范围错误。
...
在 OLED 上显示字符、字符串，并把 DHT11 读到的数据格式化后显示。

### 核心理解

OLED 不认识 C 字符串。显示字符的本质是：用字符编码查字库，把点阵字节写到显存。DHT11 数据显示只是把传感器数值转成字符，再调用显示字符串函数。
...
/* DHT11_Read 的实现见 10 章，具体接口以课程代码为准。 */
snprintf(line, sizeof(line), "T:%d H:%d", temperature, humidity);
...
理解位图显示是把取模后的字节数组写入 OLED 显存，而不是直接把 PNG/JPG 文件发给屏幕。
...
- 先显示 8x8 或 16x16 小图标，不要一开始上整屏图片。
- 图像左右/上下反了，优先检查取模方向。
- 图像错位，检查页地址和列地址。

### 常见坑

- 把图片文件当成 MCU 可直接显示的数据。
- 取模格式和显示函数不匹配。
- 宽高单位按像素和字节混用。
