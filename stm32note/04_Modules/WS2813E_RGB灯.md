...
WS2813E 接收的是按严格脉宽编码的整串颜色位流，不是普通 UART 数据，也不是单纯设置一个 PWM 占空比。刷新过程必须同时保证颜色顺序、每一位波形以及末尾 reset/latch 时段正确。
...
1. 按灯珠数量估算供电，确认共地、DIN 方向和逻辑电平。
2. 只驱动第一颗灯，以固定低亮度颜色验证数据方向。
3. 用示波器核对定时器输出的 0/1 位波形和 reset/latch 时段。
4. 分别发送固定红、绿、蓝，确认颜色字节顺序。
5. 扩展像素 buffer 和 DMA 长度，验证多灯级联。
6. 最后加入动画、限亮度和重复刷新控制。
...
- [[联网控制与总线通信]] 可把远程命令映射为颜色更新。
- 通用 bring-up 和数据显示见 [[传感器采集与显示]]。
- 跨外设排错见 [[通用调试方法]]。

## 复习检查清单

- [ ] 能先检查灯带供电预算、共地、DIN 方向和逻辑电平兼容。
- [ ] 能说明颜色 buffer、颜色字节顺序和整串位流的关系。
- [ ] 能把每个数据 bit 编码为定时器 CCR 序列，而不是普通 UART 字节。
- [ ] 能说明 TIM PWM + DMA 如何稳定输出时序并在完成后停止通道。
- [ ] 能在数据末尾保留 reset/latch 时段后再开始下一次刷新。
- [ ] 能用示波器验证波形，而不是只根据灯珠颜色猜时序。
- [ ] 能指出颜色顺序、纳秒窗口、灯珠数量和驱动方式必须实测核对。
