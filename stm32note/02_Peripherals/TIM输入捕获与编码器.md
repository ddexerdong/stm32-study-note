...
- [ ] 能区分编码器模式读取位置变化与输入捕获测量边沿时间。
- [ ] 能用周期前后 CNT 的差值计算编码器增量，并处理计数回绕。
- [ ] 能说明外部边沿到来时 CCR 锁存的是当时的 CNT。
- [ ] 能用上升沿、下降沿和两次 CCR 差值描述脉宽测量流程。
- [ ] 能把定时器计数频率换算为时间，并处理溢出扩展。
- [ ] 能说明超声波换算中的“1 MHz”只有在计数频率确实配置为 1 MHz 时成立。
- [ ] 能按输入波形、引脚复用、计数频率、边沿配置和回调顺序排查异常。
