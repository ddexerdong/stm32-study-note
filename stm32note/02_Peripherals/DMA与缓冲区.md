...
- [ ] 能说明 DMA 的方向、宽度、长度、地址递增和触发源分别控制什么。
- [ ] 能解释为什么异步 DMA buffer 不能使用已经退出作用域的局部数组。
- [ ] 能区分 Normal 与 Circular 模式的数据覆盖和停止行为。
- [ ] 能区分 UART DMA、UART IDLE + DMA 与 ADC Circular DMA 的数据边界。
- [ ] 能说明多数外设场景应调用 UART/ADC 的 DMA 封装 API，而不是直接调用 `HAL_DMA_Start()`。
- [ ] 能为 Half Complete 和 Complete 回调安排不会覆盖未处理数据的消费策略。
- [ ] 能谨慎使用 NDTR 判断剩余长度，并考虑读取时 DMA 仍在变化。
- [ ] 能用 ring buffer 或双缓冲处理生产速度与业务消费速度不一致的问题。
