...
- [ ] 能从 APB 分频推导 `TIMx_CLK`，并判断是否存在定时器时钟 x2。
- [ ] 能说明实际分频是 `PSC + 1`、计数周期是 `ARR + 1`。
- [ ] 能用 `TIMx_CLK`、PSC 和 ARR 计算 Update Event 周期。
- [ ] 能区分 `HAL_TIM_Base_Start()` 与 `HAL_TIM_Base_Start_IT()` 的使用场景。
- [ ] 能追踪 Update Event、NVIC、IRQHandler 到 `HAL_TIM_PeriodElapsedCallback()`。
- [ ] 能在多个 TIM 共用回调时判断 `htim->Instance`。
- [ ] 能按时钟、PSC/ARR、Start、NVIC 和回调链逐层排查定时器不工作。
