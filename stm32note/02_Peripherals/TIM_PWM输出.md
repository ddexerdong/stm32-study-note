...
PWM 不是软件循环拉高拉低，而是 TIM 在一个计数周期内根据 CNT、CCR 和 ARR 自动改变输出状态。它输出的是高速开关波形：LED 看平均亮度，舵机看脉宽，电机驱动看调制后的平均电压。
...
- [[WS2813E_RGB灯]] 使用定时器波形与 DMA 刷新灯珠数据。

## 复习检查清单

- [ ] 能根据 `TIMx_CLK`、PSC 和 ARR 计算 PWM 频率。
- [ ] 能说明 CCR 与 ARR 如何共同决定占空比和有效脉宽。
- [ ] 能在 CubeMX 中把通道配置为 PWM，并确认引脚处于复用输出模式。
- [ ] 能调用 `HAL_TIM_PWM_Start()` 启动正确通道。
- [ ] 能用 `__HAL_TIM_SET_COMPARE()` 更新 CCR，而不破坏周期配置。
- [ ] 能按舵机手册和实测标定供电、周期与脉宽范围。
- [ ] 能说明高级定时器主输出、死区和刹车只在相应配置下参与输出。
